...
$.fn.dataTableExt.oApi.fnExtStylePagingInfo = function(oSettings) {
	return {
		...
		"pagecountid":oSettings.sTableId+"_pagecount"
	};
};
...
$.fn.dataTableExt.oPagination.extStyle = {
	...
	"fnInit" : function(oSettings, nPaging, fnCallbackDraw) {
		...
		var pageinfo=oSettings.oInstance.fnGetData();
		nFirst = $('<a/>', {
		...
		nPageOf1 = $('<span />', {
			text : '',
			'id': oPaging.pagecountid
		});
		nTotalPages = $('<span />', {
			text : oPaging.iTotalPages
		});// $('<span />', { class : "paginate_total" , text :
		// oPaging.iTotalPages});
		/*
		nPageOf2 = $('<span />', {
			text : '页'
		});*/
		$(nPaging).addClass("dataTables_paginate paging_full_numbers");
		$(nPaging).append(nPageOf1).append(nFirst).append(nPrevious).append(
				nPageNumBox).append(nNext).append(nLast);
		...
		nNext.click(function() {
			...
			var expageinfo=oSettings.oInstance.fnExtStylePagingInfo();
		...
		nPageNumBox.change(function() {
			...
			// indexed, but
			// the UI is 1
			var oPaging = oSettings.oInstance.fnExtStylePagingInfo();// oSettings.oInstance.fnPagingInfo();
