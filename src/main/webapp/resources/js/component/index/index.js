...
$(function() {
	...
	$('#menu_toggle').click(function() {
		...
		if ($('body').hasClass('nav-md')) {
			...
			$('.sidebar-footer').hide();
			$('.active').find('.child_menu').each(function(i, obj) {
				$(obj).hide();
			})
	...
	$('[addtabs]').click(
			function() {
				Addtabs.add({
					...
					afteradd : function(currenturl) {
						//var currenturl = $(this).attr('url');
						$('.active').find('.current-page').each(function(i, obj) {
							$(obj).removeClass('current-page');
						})
						$('#sidebar-menu a[url="' + currenturl + '"]').parent(
								'li').addClass('current-page');
						$('#sidebar-menu a').filter(function() {
							return this.url == currenturl;
						}).parent('li').addClass('current-page').parent('ul')
