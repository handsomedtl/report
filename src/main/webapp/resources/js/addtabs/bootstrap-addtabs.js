...
$.fn.addtabs = function(options) {
	...
	Addtabs.options = $.extend({
		...
		iframeHeight : $(document).height() - 169, //固定TAB中IFRAME高度,根据需要自己修改
		method : 'init',
		callback : function() { //关闭后回调函数
		}
	}, options || {});

	$(Addtabs.options.monitor)
			.on(
					'click',
					'[data-addtab]',
					function() {
							...
							Addtabs
									.add({
										...
										content : Addtabs.options.content ? Addtabs.options.content
												: $(this).attr('content'),
										url : $(this).attr('url'),
										ajax : $(this).attr('ajax') ? true
												: false
									});
						} else if (window.systemType == 'portal') {
							var title = $(this).attr('title') ? $(this).attr(
									'title') : $(this).html();
							var url = $(this).attr('url');
							//TODO 菜单挂到portal下面后，以下方法需要测试。这个方法在public.js中
							parent.parent.addNavFrame('contentframe', url,
									title);
						}
					});
	...
	obj.on('mouseover', '.close-tab', function() {
		$(this).removeClass('glyphicon-remove').addClass(
				'glyphicon-remove-circle');
	})

	obj.on('mouseout', '.close-tab', function() {
		$(this).removeClass('glyphicon-remove-circle').addClass(
				'glyphicon-remove');
	})
...
window.Addtabs = {
	options : {},
	add : function(opts) {
		Addtabs.options = $.extend(Addtabs.options, opts);
		var id = 'tab_' + opts.id;
		obj.find('.active').removeClass('active');
		if (!Addtabs.beforeAdd()) {
			alert('标签页超过最大长度，请先关闭几个标签页再打开新的标签页！');
			return false;
		}
		//如果TAB不存在，创建一个新的TAB
		if (!$("#" + id)[0]) {
		...
		$("#" + id).addClass("active");
		if (opts.afteradd != undefined) {
			//alert(1);
			//afteradd
			opts.afteradd.call(this, opts.url);
		}
		//Addtabs.drop();
	},
	close : function(id) {
	...
	drop : function() {
		...
		element.append(dropdown.find('li')).find('>li').not('.tabdrop').each(
				function() {
					if (this.offsetTop > 0
							|| element.width() - $(this).position().left
									- $(this).width() < 53) {
