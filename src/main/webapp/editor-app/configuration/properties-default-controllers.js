...
var KisBpmSelectPropertyCtrl = [ '$scope','$http', function ($scope,$http) {

	/** $http.get(KISBPM.URL.getTemplatesInfo()).success(function(response) {
    ...
    $scope.inputBlurred = function() {
    	...
    	if ($scope.property.value) {
    		$scope.property.value = $scope.property.value.replace(/(<([^>]+)>)/ig,"");
    		$scope.property.valueText =  $scope.getSelectText($scope.templates,$scope.property.value);
    	}
        $scope.updatePropertyInModel($scope.property);
    };
    
    $scope.getSelectText = function(jsonArr,selectvalue) {
        var selectText = '';
