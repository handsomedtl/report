...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    	...
    	var processType = EDITOR.UTIL.getParameterByName('processType');
    	
    	//add by wlin for Select 
    	$http.get(KISBPM.URL.getTemplatesInfo(processType)).success(function(response) {
        ...
        $scope.getTempSelectText = function(jsonArr,selectvalue) {
            var selectText = '';
        	for(i = 0; i < jsonArr.length; i++) {
        		var json = jsonArr[i];
        		if(json.templateId == selectvalue){
        			selectText = json.templateName;
        		}
        	}
        ...
        $scope.getRolesSelectText = function(jsonArr,selectvalue) {
            var selectText = '';
        	for(i = 0; i < jsonArr.length; i++) {
        		var json = jsonArr[i];
        		if(json.groupfunc == selectvalue){
        			selectText = json.groupname;
        		}
        	}
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            if(currentProperty.type ==='select'){
                            	currentProperty.valueText = $scope.getTempSelectText($scope.templates,selectedShape.properties[key]);
                            }
                            
                            if(key === 'oryx-usertaskassignment'){
                            	if(selectedShape.properties[key].assignment !== undefined){
                            		if(selectedShape.properties[key].assignment.candidateUsers > 0){
                            			currentProperty.value.assignment.valueText = $scope.getRolesSelectText($scope.roleGroups,selectedShape.properties[key].assignment.candidateUsers[0].value);
                            		}
                            	}
                            }
